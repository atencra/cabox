function [stcdata, sigstcdata, proj_exc] = ca_cadata_stc_projection(cadata, stimulus)
...
if ( nargin == 1 )
    stimpath = 'C:\Users\craig\Ripple_Noise';
    sprfile = cadata.stimfile;
    stimfile = fullfile(stimpath, sprfile);
...
for i = 10:10 %size(spktrain,1)

    [priorstim, resp] = fio_stimtrial_resp_from_stimmat(stimulus, spktrain(i,:), nf, nlags, x0);
    sta = priorstim' * resp;
    ...
    evals_rand_total = [];
    for j = 1:2
        resp_rand = circshift(resp, [round((j/4)*length(resp)) 0]);
        ...
        evals_rand_total = [evals_rand_total evals_rand(:)'];
        clear('delC_rand');
    end
    ...
    stcdata.evals = evals;
    stcdata.evecs = evecs;
    stcdata.evals_rand = evals_rand_total;


    ca_plot_cadata_stc_evals_evecs(stcdata);
    pause(1);

    sigstcdata = ca_stc_sig_evals_evecs(stcdata);
    evecs_exc = sigstcdata.evecs_exc;
    proj_exc = priorstim * evecs_exc;
    proj_sd = std(proj_exc,0,1);
    proj_exc = proj_exc ./ repmat(proj_sd,size(proj_exc,1),1);
