function ca_cat_batch_calc_cell_assembly_sta_stim_binsize
% ca_cat_batch_calc_cell_assembly_sta_stim_binsize Batch estimate PCA/ICA cell assemblies and STAs
% 
...
%---------------------- to do --------------------
% save #evals outside m-p bounds
% pairwise correlations between neurons within a cell assembly



d1 = dir('*-site*-*um-*db-rn*-fs*-*-strfcmb.mat'); % for single units
d2 = dir('*-site*-*um-*db-rn*-fs*-thresh-strf.mat'); % for multi-units

d3 = dir('*-strfcmb.mat'); % for single units
...
for i = 1:length(d)
    ...
    for j = 1:length(df)
        ...
        outfile = sprintf('%s-ca-%.0fdft.mat', filename(1:end-4), df(j));
        fprintf('Outfile: %s\n', outfile);

        if ( exist(outfile, 'file') ) % skip if output already exists
            fprintf('%s already processed.', outfile);
        else
            [basefile] = ca_get_base_downsampled_spr_filename(exp);
            [stimstr] = ca_cat_get_ripple_noise_stimulus([], basefile, df(j), dff);
            [cadata] = ca_calc_cell_assembly_sta_from_spk_stimulus_trigger(...
                str.spk, stimstr.stimulus, str.trigger, stimstr.Fs, stimstr.DF);
            cadata.stimfile = stimstr.stimfile;
